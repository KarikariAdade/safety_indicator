@extends('layouts.dashboard')
@section('content')
    @push('custom-css')
        <link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/bs5/jszip-2.5.0/dt-1.12.1/b-2.2.3/b-colvis-2.2.3/b-html5-2.2.3/b-print-2.2.3/date-1.1.2/fh-3.2.4/r-2.3.0/sc-2.0.7/sb-1.3.4/sp-2.0.2/sl-1.4.0/datatables.min.css"/>

    @endpush
<div class="row">
    <div class="col-xl-12 stats_cards">
        <form class="card filterIndicator" action="{{ route('home.filter.stats') }}" style="height: auto !important;">
            @csrf
            @method('POST')
            <div class="card-body row">
                <div class="form-group col-md-4">
                    <label>From <span class="text-danger">*</span></label>
                    <input type="date" name="start_date" class="form-control input-rounded">
                </div>
                <div class="form-group col-md-4">
                    <label>To <span class="text-danger">*</span></label>
                    <input type="date" name="end_date" class="form-control input-rounded">
                </div>
                <div class="col-md-4 mt-4">
                    <button type="submit" class="btn btn-primary">Filter</button>
                </div>
            </div>
        </form>
        <div class="row">
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-body">
                        <div class="media align-items-center">
                            <div class="media-body mr-3">
                                <h2 class="num-text text-black font-w600" id="aifr">{{ $indicator[0]->aifr }}</h2>
                                <span class="fs-14">All Injury Frequency Rate</span>
                            </div>
                            <svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M34.4221 13.9831C34.3342 13.721 34.1756 13.4884 33.9639 13.3108C33.7521 13.1332 33.4954 13.0175 33.2221 12.9766L23.6491 11.5141L19.3531 2.36408C19.232 2.10638 19.04 1.88849 18.7996 1.73587C18.5592 1.58325 18.2803 1.5022 17.9956 1.5022C17.7108 1.5022 17.432 1.58325 17.1916 1.73587C16.9512 1.88849 16.7592 2.10638 16.6381 2.36408L12.3421 11.5141L2.76908 12.9766C2.49641 13.0181 2.24048 13.1341 2.02943 13.3117C1.81837 13.4892 1.66036 13.7215 1.57277 13.9831C1.48517 14.2446 1.47139 14.5253 1.53293 14.7941C1.59447 15.063 1.72895 15.3097 1.92158 15.5071L8.89808 22.6501L7.24808 32.7571C7.20306 33.0345 7.23685 33.3189 7.34561 33.578C7.45437 33.8371 7.63373 34.0605 7.86325 34.2226C8.09277 34.3847 8.36321 34.4791 8.64377 34.495C8.92432 34.5109 9.20371 34.4477 9.45008 34.3126L18.0001 29.5906L26.5501 34.3126C26.7965 34.4489 27.0762 34.5131 27.3573 34.4978C27.6385 34.4826 27.9097 34.3885 28.1399 34.2264C28.37 34.0643 28.55 33.8406 28.659 33.5811C28.7681 33.3215 28.8019 33.0365 28.7566 32.7586L27.1066 22.6516L34.0786 15.5071C34.2703 15.3091 34.4038 15.0622 34.4644 14.7933C34.525 14.5245 34.5103 14.2441 34.4221 13.9831Z"
                                    fill="#353d25"/>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-body">
                        <div class="media align-items-center">
                            <div class="media-body mr-3">
                                <h2 class="num-text text-black font-w600" id="trifr">{{ $indicator[0]->trifr }}</h2>
                                <span class="fs-14">Total Recordable Injury Frequency Rate</span>
                            </div>
                            <svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M17.8936 22.4999C23.6925 22.4999 28.3936 17.7989 28.3936 11.9999C28.3936 6.20095 23.6925 1.49994 17.8936 1.49994C12.0946 1.49994 7.39355 6.20095 7.39355 11.9999C7.39355 17.7989 12.0946 22.4999 17.8936 22.4999Z"
                                    fill="#353d25"/>
                                <path
                                    d="M29.5606 21.3344C29.2171 20.9909 28.8511 20.6699 28.4761 20.3564C27.216 21.96 25.6079 23.2562 23.7734 24.1472C21.9389 25.0382 19.926 25.5007 17.8865 25.4996C15.8471 25.4986 13.8346 25.0342 12.001 24.1414C10.1674 23.2486 8.56061 21.9507 7.30209 20.3459C5.4471 21.8906 3.95587 23.8256 2.9348 26.013C1.91373 28.2003 1.38799 30.586 1.39509 32.9999C1.39509 33.3978 1.55313 33.7793 1.83443 34.0606C2.11573 34.3419 2.49727 34.4999 2.89509 34.4999H32.8951C33.2929 34.4999 33.6744 34.3419 33.9557 34.0606C34.2371 33.7793 34.3951 33.3978 34.3951 32.9999C34.4005 30.8324 33.976 28.6854 33.1461 26.683C32.3163 24.6807 31.0976 22.8627 29.5606 21.3344Z"
                                    fill="#353d25"/>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-body">
                        <div class="media align-items-center">
                            <div class="media-body mr-3">
                                <h2 class="num-text text-black font-w600" id="ltirf">{{ $indicator[0]->ltirf }}</h2>
                                <span class="fs-14">Lost Time injury frequency Rate</span>
                            </div>
                            <svg width="36" height="36" viewBox="0 0 36 36" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M12 1.5H6C3.51472 1.5 1.5 3.51472 1.5 6V29.8125C1.5 32.2977 3.51472 34.3125 6 34.3125H12C14.4853 34.3125 16.5 32.2977 16.5 29.8125V6C16.5 3.51472 14.4853 1.5 12 1.5Z"
                                    fill="#353d25"/>
                                <path
                                    d="M30 1.5H24C21.5147 1.5 19.5 3.51472 19.5 6V12C19.5 14.4853 21.5147 16.5 24 16.5H30C32.4853 16.5 34.5 14.4853 34.5 12V6C34.5 3.51472 32.4853 1.5 30 1.5Z"
                                    fill="#353d25"/>
                                <path
                                    d="M30 19.5H24C21.5147 19.5 19.5 21.5147 19.5 24V30C19.5 32.4853 21.5147 34.5 24 34.5H30C32.4853 34.5 34.5 32.4853 34.5 30V24C34.5 21.5147 32.4853 19.5 30 19.5Z"
                                    fill="#353d25"/>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-body">
                        <div class="media align-items-center">
                            <div class="media-body mr-3">
                                <h2 class="num-text text-black font-w600" id="lti">{{ $indicator[0]->lti }}</h2>
                                <span class="fs-14">Lost Time Injury</span>
                            </div>
                            <svg width="46" height="46" viewBox="0 0 46 46" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M34.4998 1.91666H11.4998C8.95911 1.9197 6.52332 2.93035 4.72676 4.72691C2.93019 6.52348 1.91955 8.95927 1.9165 11.5V26.8333C1.91929 29.0417 2.68334 31.1816 4.07988 32.8924C5.47642 34.6031 7.42004 35.7801 9.58317 36.225V42.1667C9.58312 42.5137 9.67727 42.8542 9.85558 43.1518C10.0339 43.4495 10.2897 43.6932 10.5956 43.8569C10.9016 44.0206 11.2462 44.0982 11.5928 44.0814C11.9394 44.0645 12.2749 43.9539 12.5636 43.7613L23.5748 36.4167H34.4998C37.0406 36.4136 39.4764 35.403 41.2729 33.6064C43.0695 31.8098 44.0801 29.374 44.0832 26.8333V11.5C44.0801 8.95927 43.0695 6.52348 41.2729 4.72691C39.4764 2.93035 37.0406 1.9197 34.4998 1.91666ZM30.6665 24.9167H15.3332C14.8248 24.9167 14.3373 24.7147 13.9779 24.3553C13.6184 23.9958 13.4165 23.5083 13.4165 23C13.4165 22.4917 13.6184 22.0041 13.9779 21.6447C14.3373 21.2853 14.8248 21.0833 15.3332 21.0833H30.6665C31.1748 21.0833 31.6623 21.2853 32.0218 21.6447C32.3812 22.0041 32.5832 22.4917 32.5832 23C32.5832 23.5083 32.3812 23.9958 32.0218 24.3553C31.6623 24.7147 31.1748 24.9167 30.6665 24.9167ZM34.4998 17.25H11.4998C10.9915 17.25 10.504 17.0481 10.1446 16.6886C9.78511 16.3292 9.58317 15.8417 9.58317 15.3333C9.58317 14.825 9.78511 14.3375 10.1446 13.978C10.504 13.6186 10.9915 13.4167 11.4998 13.4167H34.4998C35.0082 13.4167 35.4957 13.6186 35.8551 13.978C36.2146 14.3375 36.4165 14.825 36.4165 15.3333C36.4165 15.8417 36.2146 16.3292 35.8551 16.6886C35.4957 17.0481 35.0082 17.25 34.4998 17.25Z"
                                    fill="#353d25"/>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6">
                <div class="card">
                    <div class="card-body">
                        <div class="media align-items-center">
                            <div class="media-body mr-3">
                                <h2 class="num-text text-black font-w600" id="damage_free">{{ $indicator[0]->damage_free }}</h2>
                                <span class="fs-14">Damage Free</span>
                            </div>
                            <svg width="46" height="46" viewBox="0 0 46 46" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M34.4998 1.91666H11.4998C8.95911 1.9197 6.52332 2.93035 4.72676 4.72691C2.93019 6.52348 1.91955 8.95927 1.9165 11.5V26.8333C1.91929 29.0417 2.68334 31.1816 4.07988 32.8924C5.47642 34.6031 7.42004 35.7801 9.58317 36.225V42.1667C9.58312 42.5137 9.67727 42.8542 9.85558 43.1518C10.0339 43.4495 10.2897 43.6932 10.5956 43.8569C10.9016 44.0206 11.2462 44.0982 11.5928 44.0814C11.9394 44.0645 12.2749 43.9539 12.5636 43.7613L23.5748 36.4167H34.4998C37.0406 36.4136 39.4764 35.403 41.2729 33.6064C43.0695 31.8098 44.0801 29.374 44.0832 26.8333V11.5C44.0801 8.95927 43.0695 6.52348 41.2729 4.72691C39.4764 2.93035 37.0406 1.9197 34.4998 1.91666ZM30.6665 24.9167H15.3332C14.8248 24.9167 14.3373 24.7147 13.9779 24.3553C13.6184 23.9958 13.4165 23.5083 13.4165 23C13.4165 22.4917 13.6184 22.0041 13.9779 21.6447C14.3373 21.2853 14.8248 21.0833 15.3332 21.0833H30.6665C31.1748 21.0833 31.6623 21.2853 32.0218 21.6447C32.3812 22.0041 32.5832 22.4917 32.5832 23C32.5832 23.5083 32.3812 23.9958 32.0218 24.3553C31.6623 24.7147 31.1748 24.9167 30.6665 24.9167ZM34.4998 17.25H11.4998C10.9915 17.25 10.504 17.0481 10.1446 16.6886C9.78511 16.3292 9.58317 15.8417 9.58317 15.3333C9.58317 14.825 9.78511 14.3375 10.1446 13.978C10.504 13.6186 10.9915 13.4167 11.4998 13.4167H34.4998C35.0082 13.4167 35.4957 13.6186 35.8551 13.978C36.2146 14.3375 36.4165 14.825 36.4165 15.3333C36.4165 15.8417 36.2146 16.3292 35.8551 16.6886C35.4957 17.0481 35.0082 17.25 34.4998 17.25Z"
                                    fill="#353d25"/>
                            </svg>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header mb-0 d-sm-flex flex-wrap d-block shadow-sm border-0 align-items-center">
                        <div class="mr-auto pr-3 mb-3">
                            <h4 class="text-black fs-20 mb-sm-0 mb-2">Report</h4>
                        </div>
                        <div class="card-action card-tabs mb-3">
                            <ul class="nav nav-tabs" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link group_filter" data-toggle="tab" href="#" title="all"
                                       aria-selected="false">
                                        All
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link group_filter" data-toggle="tab" href="#" title="daily"
                                       aria-selected="false">
                                        Today
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link group_filter" data-toggle="tab" href="#" title="weekly">
                                        This Week
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link group_filter" data-toggle="tab" href="#" title="monthly">
                                        This Month
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            {!! $dataTable->table(['class' => 'text-center table-hover table-striped']) !!}
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

</div>
<div class="modal fade addModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">New Indicators</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form class="row addIndicator" method="POST" action="{{ route('indicator.store') }}">
                    @csrf
                    @method('POST')
                    <div class="form-group col-md-4">
                        <label>AIFR <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" step="0.01" name="aifr">
                    </div>
                    <div class="form-group col-md-4">
                        <label>TRIFR <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" step="0.01" name="trifr">
                    </div>
                    <div class="form-group col-md-4">
                        <label>LTIRF <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" step="0.01" name="ltirf">
                    </div>
                    <div class="form-group col-md-4">
                        <label>LTI <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" step="0.01" name="lti">
                    </div>
                    <div class="form-group col-md-6">
                        <label>Equipment Damage Free Days <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" name="damage_free">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger light" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>

        </div>
    </div>
</div>
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Update Indicator</h5>
                    <button type="button" class="close" data-dismiss="modal"><span>&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form class="row addIndicator" id="editIndicator" method="POST">
                        @csrf
                        @method('POST')
                        <div class="form-group col-md-4">
                            <label>AIFR <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" step="0.01" name="aifr" id="edit_aifr">
                        </div>
                        <div class="form-group col-md-4">
                            <label>TRIFR <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" step="0.01" name="trifr" id="edit_trifr">
                        </div>
                        <div class="form-group col-md-4">
                            <label>LTIRF <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" step="0.01" name="ltirf" id="edit_ltirf">
                        </div>
                        <div class="form-group col-md-4">
                            <label>LTI <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" step="0.01" name="lti" id="edit_lti">
                        </div>
                        <div class="form-group col-md-6">
                            <label>Equipment Damage Free Days <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" name="damage_free" id="edit_damage_free">
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger light" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-primary">Update changes</button>
                        </div>
                    </form>
                </div>

            </div>
        </div>
    </div>
@endsection
@push('custom-js')
    <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.36/pdfmake.min.js"></script>
    <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/pdfmake/0.1.36/vfs_fonts.js"></script>
    <script type="text/javascript" src="https://cdn.datatables.net/v/bs5/jszip-2.5.0/dt-1.12.1/b-2.2.3/b-colvis-2.2.3/b-html5-2.2.3/b-print-2.2.3/date-1.1.2/fh-3.2.4/r-2.3.0/sc-2.0.7/sb-1.3.4/sp-2.0.2/sl-1.4.0/datatables.min.js"></script>

    {!! $dataTable->scripts() !!}
    <script>
        // $(document).bind("contextmenu",function(e){
        //     return false;
        // });
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
        });

            let group_filter = $('.group_filter');

            const datatable = $('#dataTable');
            $('#generate').on('click', function () {
                datatable.on('preXhr.dt', function (e, settings, data) {
                    data.from = $('#from').val();
                    data.to = $('#to').val();
                });
                datatable.DataTable().ajax.reload();
                return false;
            });

            group_filter.on('click', function (e){
                e.preventDefault()

                let attr = $(this).attr('title');

                removeAttr();

                $(this).prop('active', true);

                datatable.on('preXhr.dt', function (e, settings, data) {
                    data.filter = attr;
                });
                datatable.DataTable().ajax.reload();
                // return ;

            })

            function removeAttr(){
                group_filter.removeAttr('active');
            }

            function runToast(msg, code){

                if (code == '200'){
                    Notiflix.Notify.success(msg);
                }else{
                    Notiflix.Notify.failure(msg);
                }
            }

            function refreshStats(data){

                $('#aifr').html(Number(data[0].aifr).toFixed(2))

                $('#trifr').html(Number(data[0].trifr).toFixed(2))

                $('#ltirf').html(Number(data[0].ltirf).toFixed(2))

                $('#lti').html(Number(data[0].lti).toFixed(2))

                $('#damage_free').html(Number(data[0].damage_free).toFixed(2))
                console.log('this is the data',data, data[0].aifr)
            }

            $('.filterIndicator').submit(function (e) {
                e.preventDefault();

                Notiflix.Block.pulse('.stats_cards');

                $.ajax({
                    url: $(this).attr('action'),
                    method: 'POST',
                    data: $(this).serialize()
                }).done(function (response) {
                    if(response.code === 200){

                        refreshStats(response.msg)

                    }else{
                        runToast(response.msg, response.code)
                    }
                })

                Notiflix.Block.remove('.stats_cards');
            })



            $('.addIndicator').submit(function (e) {
                e.preventDefault();

                Notiflix.Block.pulse('.addIndicator');

                let url = $(this).attr('action');

                formData = new FormData(this);
                $.ajax({

                    url: url,
                    method: 'POST',
                    data: formData,
                    dataType: 'JSON',
                    contentType: false,
                    cache: false,
                    processData: false,

                }).done((response) => {

                    console.log('add indicator response: ', response)

                    if (response.code === 200) {

                        runToast(response.msg, response.code)

                        refreshStats(response.data)

                        setTimeout(function () {

                            $('.addIndicator').closest('.modal').modal('hide');

                        }, 1000)

                    }else{
                        runToast(response.msg, response.code)
                    }
                })

                Notiflix.Block.remove('.addIndicator');
            })


        $('#dataTable').on('click', '#deleteIndicator', function (e){
            e.preventDefault();
            runAjaxPrompt($(this).attr('href'));
            updateData()
        });


        function runAjaxPrompt(url, type = null){
            Notiflix.Block.pulse('#dataTable');
            let msg = 'You won\'t be able to revert this!';


            Notiflix.Confirm.show(
                'Are You Sure?',
                msg,
                'Yes, Delete',
                'No',
                function okCb() {
                    $.post(url, function (response){
                        console.log(response)
                        if(response.code == '200'){
                            Notiflix.Report.success(
                                'Success',
                                response.msg,
                                'Okay',
                            );
                            $('#dataTable').DataTable().ajax.reload();

                        }else{
                            runToast(response.msg, response.code)
                        }
                    });


                },
                {
                    width: '320px',
                    borderRadius: '8px',
                },
            );
            Notiflix.Block.remove('#dataTable');

        }

        function updateData(){
            $.ajax({
                url: '{{ route('welcome') }}',
                method: 'POST'
            }).done((response) => {
                console.log('delete repsonse', response)
                if(response.code === 200){
                    refreshStats(response.msg)
                }
            })
        }

    </script>
@endpush
